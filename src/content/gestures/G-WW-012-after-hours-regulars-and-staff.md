---
id: G-WW-012
plateau: P-WW
title: After Hours, Regulars and Staff
slug: after-hours-regulars-and-staff
tags:
  - plateau/P-WW
  - voice/jason
  - kind/scene
  - capacity/read
  - status/draft
  - direction/e
  - zone/the-wizards-cottage
  - place/norfolk-va
  - concept/commons
  - image/the-restaurant
  - motif/dissolving-boundary
  - emotion/tenderness
  - practice/gathering
beauty_check:
  passed: false
  notes: |
    2026-05-23: Draft. Scene-setting for WW-01. The restaurant is
    referenced across the book (P-OA, P-GC, P-AB); this gesture does
    not re-narrate it, just sets the after-hours room. The student is
    a staff member; the room is regulars-and-staff. Norfolk implicit.
    2026-05-23 (Jason): correction — staff member was clocked out
    with their shift drink, sitting in among the regulars. That is
    itself a nod to the commons. Reworked the scene so the line
    between staff and regular at that hour is the line the bar was
    quietly dissolving.
notes: |
  Direction E — the social east, the room. Place tag: norfolk-va.
  Hold the scene loose; the action lives in the next two gestures.
---
The doors had been locked behind the last guest. The lights had been turned down to the after-hours level — bright enough to count, dim enough to talk. A few regulars were at the bar, the way regulars stay after hours when the room belongs to them as much as to anyone. The staff who were still around were clocked out, shift drinks in hand, sitting in among the regulars — which is the commons before any of us would have called it that. I was behind the bar, polishing glasses. There was a conversation going.
